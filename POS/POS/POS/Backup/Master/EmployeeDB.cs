...
# endregion
		...
		public int SaveFunction(Saving.SaveType Mode)
		{
			...
			try
			{
				...
				Con.Open(); 
				SqlCommand Cmd;
                Cmd = new SqlCommand("EmployeeSP", Con); 
				Cmd.CommandType=CommandType.StoredProcedure; 
				SqlParameter param=new SqlParameter("@mode",SqlDbType.TinyInt);
				param.Value=Modeval;
				Cmd.Parameters.Add(param);

                param = new SqlParameter("@EmpId", SqlDbType.SmallInt);
                param.Value = _EmployeeId;
				Cmd.Parameters.Add(param);

                param = new SqlParameter("@EmpName", SqlDbType.VarChar, 50);
                param.Value = _EmpName.Replace("'", "''");
				Cmd.Parameters.Add(param);
		...
		public void SetViewControl()
		{
            ...
            _View.TableNames = "EmployeeFn(" + Common.CId + ")";			
			_View.Fields ="*";			
			_View.Conditions = "";
		}
        ...
        public DataTable Employee_Source()
        {
            ...
            Con.Open();
            SqlCommand Cmd;
            Cmd = new SqlCommand("select * from EmployeeFn(" + Common.CId + ") where EmpId > 0 ", Con);
            Cmd.CommandType = CommandType.Text;

            Da.SelectCommand = Cmd;
            Da.Fill(Tbl);
            Tbl.TableName = "Employee";
            Da.Dispose();
            Cmd.Dispose();
            Con.Close();
        ...
        public int Employee_GridStyle(DataGridViewColumnCollection tmp)
        {
            ...
            tmp.Add(CommonView.GetGridViewColumn("EmpName", "Name", 120, 2, false, DataGridViewContentAlignment.MiddleLeft, "", DataGridViewAutoSizeColumnMode.Fill));
